...
def mcap_to_csv(bag_path, out_name):
    ...
    make_motor_status_csv(path_motor)
...
def make_motor_status_csv(path):
    execute_start_time_path = Path("/home/mgazzola/ros2_spirob_ws/var/execute_start_time.json")
    motor_profile_path = Path("/home/mgazzola/ros2_spirob_ws/var/motor_profile.json")
    ...
    #Load data from the pathes
    with open(execute_start_time_path,"r") as f:
        start_time = json.load(f)
    with open(motor_profile_path, "r") as f:
        motor_profile = json.load(f)
    
    #Extrating timeline information
    combined_timeline = []
    last_time_motors = []

    for i in range(len(motor_profile)):
        last_time_motors.append(max(motor_profile[i][0]))

    last_time  = max(last_time_motors)   
        ...
        for i in range(len(motor_profile)):
            ...
            for time_idx, time_value in enumerate(motor_profile[i][0]):
                if time_value <= time:
                    ...
                    row[f"motor_status{i+1}"] = int(step)
...
if __name__ == "__main__":
    ...
    path_motor = Path("/home/mgazzola/ros2_spirob_ws/exp_ws/src/test.csv")
    make_motor_status_csv(path_motor)
